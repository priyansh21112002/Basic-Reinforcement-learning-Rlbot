...
from rlgym.utils.reward_functions.common_rewards.ball_goal_rewards import LiuDistanceBallToGoalReward,BallYCoordinateReward
from rlgym.utils.reward_functions.common_rewards.player_ball_rewards import FaceBallReward,TouchBallReward
...
if __name__ == '__main__':  # Required for multiprocessing
    ...
    def exit_save(model):
        model.save(add+"models/exit_save")
    ...
    def get_match():  # Need to use a function so that each instance can call it and produce their own objects
        return Match(
            ...
            (1.0, 0.1, 1.0,0.5,0.5,1.0,0.2,1.0,1.0)),
    ...
    env = VecNormalize(env, norm_obs=True, gamma=gamma)  # Highly recommended, normalizes rewards

    try:
        model = PPO.load(
            add+"models/exit_save.zip",
            env,
            device="cuda",
        ...
        model = PPO(
            ...
            verbose=0,                   # Print out all the info as we're going
    ...
    try:
        ...
        while True:
            print(f"Current training step count = {model.num_timesteps}")
        ...
        print(f" <EXIT> Current training step count = {model.num_timesteps}")
